...
static void ui_draw_rect(
   ...
   assert( total_y_test <= win->area.h );
...
void ui_window_free( struct UI_WINDOW* win ) {
   assert( win->ui == &global_ui );
   assert( UI_SENTINAL_WINDOW == win->sentinal );
...
static
int ui_poll_keys( struct UI_WINDOW* win, struct INPUT* p ) {
   struct UI_CONTROL* control = NULL;
   int bstr_result = 0;
   int input_length = 0;
   ...
   case UI_CONTROL_TYPE_SPINNER:
      numbuff = (int*)(control->self.attachment);
      switch( p->scancode ) {
...
unsigned int ui_control_get_draw_width(
   const struct UI_CONTROL* control
) {
   unsigned int control_w = 0;
...
unsigned int ui_control_get_draw_height(
   const struct UI_CONTROL* control
) {
...
static void ui_control_draw_spinner(
   ...
   GRAPHICS* g = win->element;
   bstring numbuf = NULL;
   unsigned int* num = spinner->self.attachment;
